...
ALTER TABLE dcdm5.dis_inf
ADD COLUMN gene_accession_id_nulled varchar(100) AFTER gene_accession_id
...
#### However, to achieve the above actions, we need to first drop the row_id2 as row_id2 keep the entire row unique.


# Delete the entire rows if one of the following criteria is met:
# Criteria 1. The gene_accession_id is exactly ''
...
SELECT gene_accession_id FROM dcdm5.gene_table
)
