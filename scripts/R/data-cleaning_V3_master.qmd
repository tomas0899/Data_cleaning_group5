...
#desired order of columns established
...
for (path in all_data){ # for each file in the data
  ...
  #transpose the file data as a dataframe into dfs
...
#Ensure all column headers are lowercaseu
...
viable_strains <- c(NA, "C57BL")
merged_df$mouse_strain[!merged_df$mouse_strain %in% viable_strains] <- NA
#mouse strain NOT in viable strain - then turn into NA
...
ipd_csv[] <- lapply(ipd_csv, function(x) { # loops over function and x represents each columns (lapply treats ipd_csv as a list of columns)
  if (is.character(x)) x[x == "NA"] <- "" # checks if cholumn x is type of character - character can have 'NA' as string instead of factor
